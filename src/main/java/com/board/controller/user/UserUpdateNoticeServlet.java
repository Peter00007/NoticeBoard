...
public class UserUpdateNoticeServlet extends HttpServlet {
    private static final Logger LOGGER = Logger.getLogger(UserUpdateNoticeServlet.class);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Notice notice = noticeService.getById(Integer.parseInt((String) session.getAttribute("noticeId")));
        request.setAttribute("notice", notice);
        List<Type> types = typeService.getAllType();
        request.setAttribute("types", types);
        RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/user/updateNotice.jsp");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (!description.isEmpty() && !id.isEmpty() && types != null) {
            if (error) {
                ...
                noticeService.deleteFromNoticeType(notice);
                Notice createdNotice = noticeService.update(notice);
                ...
                session.setAttribute("message", "You have successfully updated notice");
                session.removeAttribute("noticeId");
                response.sendRedirect(request.getContextPath() + "/user/notices");
                LOGGER.info("Updated Notice by User");
            } else {
                LOGGER.info("Incorrect data for updating Notice by User");
